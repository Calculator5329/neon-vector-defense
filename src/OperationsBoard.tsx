      ...
      <div className="ops-shop">
        ...
        <div className="palette-row">
          {PALETTES.map((p) => {
            ...
            return (
              <button key={p.id} className={`palette-chip ${equipped ? 'equipped' : ''}`} disabled={purchasing !== null || (!owned && !afford)}
                title={paletteLabel}
                ...
                onClick={async () => {
                  if (owned) {
                    ...
                    setPurchasing(p.id);
                    try {
                      const granted = await purchaseEntitlement(`palette-${p.id}`);
                      meta.recordServerEntitlement(`palette-${p.id}`, granted.salvageBalance);
                      meta.equip('accent', p.id);
                      applyAccent();
                      ...
                      pushStatus({ kind: 'err', text: `${p.name} purchase could not be completed. Please try again.` });
                      sfx.error();
                    } finally {
                      setPurchasing(null);
                    }
