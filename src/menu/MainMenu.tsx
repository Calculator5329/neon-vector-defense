...
function CommanderDossierRail({ onOpenOps, onUnlockYakkob }: { onOpenOps: () => void; onUnlockYakkob?: () => void }) {
  ...
  return (
    <aside className="commander-rail" data-testid="commander-rail">
      ...
      <ReplayOfTheDayCard />
      {onUnlockYakkob && <YakkobDwarf onUnlock={onUnlockYakkob} />}
    </aside>
  );
}
...
function SectorAtlas(props: {
  ...
  const openWeeklyOps = () => {
    // Select the challenge currently occupying the dock's weekly slot.
    setDockTab('challenges');
    props.setDeployMode(props.yakkobUnlocked ? 'yakkob' : 'weekly');
    sfx.click();
    requestAnimationFrame(() => {
      weeklyRef.current?.scrollIntoView({ block: 'nearest', inline: 'nearest' });
      const target = props.yakkobUnlocked ? 'yakkob-card' : 'weekly-mutation-card';
      weeklyRef.current?.querySelector<HTMLButtonElement>(`[data-testid="${target}"]`)?.focus();
    });
  };
  ...
  return (
    <div className="menu-root">
      ...
      <div className={`menu-layout menu-tab-${tab}`}>
        <CommanderDossierRail
          onOpenOps={() => { setTab('ops'); sfx.click(); }}
          onUnlockYakkob={tab === 'deploy' && !props.yakkobUnlocked
            ? () => { props.onUnlockYakkob(); setDeployMode('yakkob'); }
            : undefined}
        />
