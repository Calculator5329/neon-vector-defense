...
export function HowToPlay({ onDone }: { onDone: () => void }) {
  const steps: [string, string, string][] = [
    ['🎯', 'Build the grid', 'Pick a tower from the ARSENAL (or press 1–9/0), then click open ground or use the arrow keys and Enter. Towers fire automatically at anything in range.'],
    ['⌬', 'Spend your credits', 'Every hull you destroy pays out. Bank it into more towers and upgrades.'],
    ['▲', 'Two upgrade tracks', 'Click a built tower to upgrade it down two paths. After your first campaign win, VETERAN DEPLOY can build and auto-upgrade a fresh tower through tier 4/4 when credits allow.'],
