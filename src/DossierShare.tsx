  ...
  return (
    <div className={`dossier-share ${compact ? 'compact' : ''}`} aria-busy={busy}>
      ...
      <div className="dossier-actions">
        {hasShare && <button className="start-btn small" disabled={busy} onClick={onShare}>⤴ SHARE</button>}
        <button className="start-btn small ghost" disabled={busy} onClick={onCopyCard}>⧉ COPY CARD</button>
        {url && <button className="start-btn small ghost" disabled={busy} onClick={onCopyLink}>🔗 COPY LINK</button>}
        <button className="start-btn small ghost" disabled={busy} onClick={onDownload}>⭳ SAVE</button>
        {toast && <span className={`dossier-toast ${toast.kind}`} role={toast.kind === 'err' ? 'alert' : 'status'}>{toast.text}</span>}
      </div>
    </div>
  );
}
