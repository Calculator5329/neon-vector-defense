...
async function installQaNetwork(page: Page) {
  await page.route((url) => url.pathname === '/src/game/balanceConfig.ts', (route) => fulfillPatched(
    route,
    replacementFor('loadRemoteBalance', `{
  return;
}`),
  ));
  await page.route((url) => url.pathname === '/src/game/dailyChallenge.ts', (route) => fulfillPatched(
    route,
    replacementFor('loadRemoteDailyOverride', `{
  return;
}`),
  ));
  await page.route((url) => url.pathname === '/src/game/weeklyChallenge.ts', (route) => fulfillPatched(route, (source) => {
    let patched = source;
  ...
  await page.route((url) => url.pathname === '/src/game/replaySpotlight.ts', (route) => fulfillPatched(
    route,
  ...
  await page.route((url) => url.pathname === '/src/game/leaderboard.ts', (route) => fulfillPatched(route, (source) => {
    let patched = source;
