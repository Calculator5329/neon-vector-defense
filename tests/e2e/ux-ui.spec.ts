...
test.describe('desktop UX layout', () => {
  ...
  test('sector atlas keeps reachable nodes, dock, and utility buttons stable', async ({ page }) => {
    ...
    expect(layout.nodes).toHaveLength(ALL_MAPS.length);
  ...
  test('sector atlas selection, keyboard path, weekly beacon, locks, and deploy launch work', async ({ page }) => {
    for (const viewport of [{ width: 1440, height: 900 }, { width: 844, height: 390 }]) {
      ...
      await expect(page.locator('.deploy-bar-sel')).toContainText('THE YAKKOB');
  ...
  for (const granted of [true, false]) {
    test(`signal palette purchase ${granted ? 'confirms a server grant' : 'preserves ownership after server rejection'}`, async ({ page }) => {
      await seedProgress(page);
      await seedMeta(page, {
        salvage: 400,
        salvageLifetime: 400,
      ...
      let requests = 0;
      await page.route('**/purchaseCosmeticEntitlement', async (route) => {
        const headers = {
          'access-control-allow-origin': '*',
          'access-control-allow-headers': '*',
          'access-control-allow-methods': 'POST, OPTIONS',
        ...
        expect(route.request().postDataJSON().data.cosmeticId).toBe('palette-void');
        requests++;
        await route.fulfill({ status: 200, headers, contentType: 'application/json', body: JSON.stringify({
          result: { granted, cosmeticIds: granted ? ['palette-void'] : [], salvageBalance: granted ? 0 : 400 },
        }) });
      });
      await openOperationsMenu(page);
      ...
      if (granted) {
        ...
        await expect(page.locator('.ops-status.err')).toContainText('Void Violet purchase could not be completed.');
        await expect(page.getByRole('button', { name: 'Buy Void Violet palette for 400 salvage' })).toBeEnabled();
      }
...
test.describe('run telemetry model', () => {
  ...
  test('weekly ops are discoverable and gauntlet absence is stable', async ({ page }) => {
    await seedMeta(page, { yakkobUnlocked: false });
    await openConsentedMenu(page);
    await page.getByTestId('dock-tab-challenges').click();
    ...
    await expect(page.getByTestId('weekly-ops-strip')).toBeVisible();
    await expect(page.getByTestId('weekly-gauntlet-card')).toContainText('Not crowned yet');
    await expect(page.getByTestId('yakkob-dwarf')).toBeVisible();
    await page.emulateMedia({ reducedMotion: 'reduce' });
    await page.getByTestId('yakkob-dwarf').click();
    await expect(page.getByTestId('yakkob-card')).toBeVisible();
